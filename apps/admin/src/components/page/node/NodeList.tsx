...
import { FunctionField, List, TextField, TextInput, BooleanField } from 'react-admin';
...
const postFilters = [
  ...
  <TextInput key="totalQuantity" name="totalQuantity" source="totalQuantity" label="总数量" />,
  <TextInput key="purchasedQuantity" name="purchasedQuantity" source="purchasedQuantity" label="已购买数量" />,
  ...
  <List filters={postFilters}>
    <MyDatagridConfigurable>
      ...
      <FunctionField
        source="limit"
        label="可领取次数"
        render={(record) => convertSmallToLarge(record.limit.toString(), TOKEN_DECIMAL)}
      />
      <FunctionField
        source="price"
        label="节点价格"
        render={(record) => convertSmallToLarge(record.price.toString(), TOKEN_DECIMAL)}
      />
      <FunctionField
        source="totalQuantity"
        label="总数量"
        render={(record) => convertSmallToLarge(record.totalQuantity.toString(), TOKEN_DECIMAL)}
      />
      <FunctionField
        source="purchasedQuantity"
        label="已购买数量"
        render={(record) => convertSmallToLarge(record.purchasedQuantity.toString(), TOKEN_DECIMAL)}
      />
